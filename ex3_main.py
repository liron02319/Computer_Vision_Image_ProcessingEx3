...
def compareLK(img_path):
   ...
   # st = time.time() #time start
    #pts, uv = opticalFlow(img_1.astype(np.float), img_2.astype(np.float), step_size=20, win_size=5)
   # et = time.time()  #time end
   # print("Time opticalFlowCompare: {:.4f}".format(et - st))

    #saveOpticalFlow("opticalFlowCompare",img_2, pts, uv)


   # st = time.time()  # time start
   # hierarchicalkDemo(img_path)
   # et = time.time()  # time end
   # print("Time opticalFlowPyrLKCompare: {:.4f}".format(et - st))

    #img1COLOR = cv2.imread('opticalFlowCompare.png', cv2.IMREAD_COLOR)
    #img1GRAY = cv2.imread('opticalFlowCompare.png', cv2.IMREAD_GRAYSCALE)
    #img2COLOR = cv2.imread('opticalFlowPyrLKCompare.png', cv2.IMREAD_COLOR)
   # img2GRAY = cv2.imread('opticalFlowPyrLKCompare.png', cv2.IMREAD_GRAYSCALE)

    #print("MSE: {}".format(MSE(img1COLOR, img2COLOR)))
   # print("Max Error: {}".format(np.abs(img1COLOR - img2COLOR).max()))
...
def findTranslationLKTest(img_path): # 3.1

    print("findTranslationLK Test")
   ...
   # img_1 = cv2.cvtColor(cv2.imread(img_path), cv2.COLOR_BGR2RGB)
    ...
    cv2.imwrite('myImgTest/imTransA1.jpg', img_2)
    st = time.time()
...
def findRigidLKTest(img_path): # 3.2

    print("findRigidLK Test")
    ...
    t=t1@t2
    img_2 = cv2.warpPerspective(img_1, t, (img_1.shape[1], img_1.shape[0]))
    cv2.imwrite('myImgTest/imRigidA1.jpg', img_2)
...
def findTranslationCorrTest(img_path): # 3.3
    print("findTranslationCorr Test")
    img_1 = cv2.cvtColor(cv2.imread(img_path), cv2.COLOR_BGR2GRAY)
    img_1 = cv2.resize(img_1, (0, 0), fx=.5, fy=0.5)
    t = np.array([[1, 0, -5],
                  [0, 1, 7],
                  [0, 0, 1]], dtype=np.float)
    img_2 = cv2.warpPerspective(img_1, t, (img_1.shape[1],img_1.shape[0]))
    cv2.imwrite('myImgTest/imTransA2.jpg', img_2)
...
def findRigidCorrTest(img_path): # 3.4
    print("findRigidCorr Test")
    img_1 = cv2.cvtColor(cv2.imread(img_path), cv2.COLOR_BGR2GRAY)
    img_1 = cv2.resize(img_1, (0, 0), fx=.5, fy=0.5)
    ...
    img_2 = cv2.warpPerspective(img_1, t, img_1.shape[::-1])
    cv2.imwrite('myImgTest/imRigidB2.jpg', img_2)
    st = time.time()
...
def main():
    ...
    #*********my tests(i didnt change the other tests)****************
    my_img_path = 'myImgTest/cat.jpg'
    findTranslationLKTest(my_img_path)
    findRigidLKTest(my_img_path)
    findTranslationCorrTest(my_img_path)
    findRigidCorrTest(my_img_path)
