...
def opticalFlow(im1: np.ndarray, im2: np.ndarray, step_size=10,
    ...
    ker = np.array([[1, 0, -1]])
    imageX = cv2.filter2D(im2, -1, ker, borderType=cv2.BORDER_REPLICATE)
    imageY = cv2.filter2D(im2, -1, ker.T, borderType=cv2.BORDER_REPLICATE)
    imageT = im2 - im1
    s = win_size // 2 # calculate
    ...
    for i in range(start, im1.shape[0] - start, step_size):
        for j in range(start, im1.shape[1] - start, step_size):
           ...
           # b = np.array([[-1 * sum(Nx[k] * Nt[k] for k in range(len(Nx))),
            ...
            AA = np.array([[(imageX[i - s: i + s + 1, j - s: j + s + 1] * imageX[i - s: i + s + 1,j - s: j + s + 1]).sum(), (imageX[i - s: i + s + 1, j - s: j + s + 1] * imageY[i - s: i + s + 1,j - s: j + s + 1]).sum()],
                            [(imageX[i - s: i + s + 1, j - s: j + s + 1] * imageY[i - s: i + s + 1,j - s: j + s + 1]).sum(), (
                                         imageY[i - s: i + s + 1, j - s: j + s + 1] * imageY[i - s: i + s + 1,j - s: j + s + 1]).sum()]])
            BB = np.array([(imageX[i - s: i + s + 1, j - s: j + s + 1] * imageT[i - s: i + s + 1, j - s: j + s + 1]).sum(), (
                                        imageY[i - s: i + s + 1, j - s: j + s + 1] * imageT[i - s: i + s + 1,j - s: j + s + 1]).sum()])
...
def opticalFlowPyrLK(img1: np.ndarray, img2: np.ndarray, k: int,
    ...
    for i in range(k - 1):
        matrixA.append(cv2.pyrDown(matrixA[-1], (matrixA[-1].shape[0] // 2, matrixA[-1].shape[1] // 2)))
        matrixB.append(cv2.pyrDown(matrixB[-1], (matrixB[-1].shape[0] // 2, matrixB[-1].shape[1] // 2)))

    matrixC = []  # make the pyramid of the change
    for i in range(len(matrixA)):
        a = matrixA[i].shape[0] #shape
        b = matrixA[i].shape[1] #shape
        change = np.zeros((a, b, 2))
        old, new = opticalFlow(matrixA[i], matrixB[i], step_size=stepSize, win_size=winSize)
...
def findRigidLK(im1: np.ndarray, im2: np.ndarray) -> np.ndarray:
    ...
    for n in range(new.shape[0]):  # see all the v,u
        ...
        t = np.array([[np.cos(theta), -np.sin(theta), 0],  # make new img transformation using theta
                      [np.sin(theta), np.cos(theta), 0],
                      [0, 0, 1]], dtype=np.float)

        newpic = cv2.warpPerspective(im1, t, (im1.shape[1], im1.shape[0]))

        d = ((im2 - newpic) ** 2).sum()  # find the difference in image
        if d < diff:
            diff = d
            spot = n
        if diff == 0:
...
def findTranslationCorr(im1: np.ndarray, im2: np.ndarray) -> np.ndarray:
    ...
    # getting the middle of the window
    I = []
    J = []
    for x in range(1, 5):
            ...
            for i in range(0, im2.shape[0]):
                for j in range(0, im2.shape[1]):
                    if (i + pad + win) < im2pad.shape[0] and (j + pad + win) < im2pad.shape[1]:
                        ...
                        if bottom != 0:   # try to find the correlation between this window and template
                            corr = top / bottom
                            if corr > big[0][0]:
                                big.clear()
                                ...
                                big.insert(0, (corr, i, j))
            if big[0][0][0] > corr_listt[0][0][0]:             # check if the first value in big is bigger than the first value (if yes- copy the values from big and add the x y vaues of the original image
                corr_listt.clear()
                for m in range(len(big)):
                    corr_listt.append((big[m], (I[x], J[y])))
            if big[0][0][0] == corr_listt[0][0][0]:  # copy the values from big and add the x y vaues of the original image)
                for m in range(len(big)):
                    corr_listt.append((big[m], (I[x], J[y])))
...
#help function(from Ex2) for gaussianPyr function
...
#help function
def kernelGaussian(kernel_size: int):
    """
    kernelGaussian
...
def pyrBlend(img_1: np.ndarray, img_2: np.ndarray,
    ...
    if len(img_1.shape) == 3 or len(img_2.shape) == 3:  # check if the image is RGB
        ...
        im_blend = pyrBlend_helper(img_1, img_2, mask, levels)  # check if the image is grayscale
